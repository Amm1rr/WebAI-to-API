...
def print_server_info(host: str, port: int, mode: str):
    ...
    colored_instructions = f"{Colors.BOLD}{Colors.YELLOW}{instruction_text.center(80)}{Colors.RESET}"
...
if __name__ == "__main__":
    ...
    if webai_is_available:
        print(f"INFO:     ✅ {Colors.CYAN}WebAI-to-API mode is available{Colors.RESET} (Gemini client initialized).")
    else:
        print(f"WARN:     ⚠️ {Colors.YELLOW}WebAI-to-API mode is not available{Colors.RESET} (Could not initialize Gemini client).")
    ...
    try:
        ...
        while True:
            ...
            if not current_process or (requested and requested != current_mode):
                ...
                target_func, process_args = (start_webai_server, (args.host, args.port, args.reload)) if current_mode == 'webai' else (start_g4f_server, (args.host, args.port))

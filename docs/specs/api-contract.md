...
### 9.1 Gemini Conversation ID Semantics

Gemini `conversation_id` tokens are treated as **opaque tokens** that instruct the selected backend to attempt continuation using its native recovery mechanism.
...
- **Playwright Continuity**: Uses provider-side Gemini conversation identifiers and URL-backed recovery mechanisms to attempt continuation of existing Gemini conversations.
- **Ownership Validation**: To prevent cross-backend routing errors (e.g., passing a WebAPI-only token to the Playwright backend), the system performs internal ownership validation:
  - If a `conversation_id` is found in the **SQLite repository**, it is strictly owned by the **WebAPI** backend.
  - Using a WebAPI-owned ID with a Playwright-based model will return a `400 Bad Request`.
  - IDs not present in SQLite are not considered WebAPI-owned and are therefore eligible for Playwright continuation attempts.
- **Backward Compatibility**: Legacy IDs previously generated with `wa_` or `pw_` prefixes are supported. The system strips these prefixes internally for processing while using the prefixed version for database lookups to ensure no continuity loss during the transition.

Cross-backend conversation continuity (e.g., continuing a WebAPI conversation in Playwright) is not supported due to incompatible underlying state formats.
